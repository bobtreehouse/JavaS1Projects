...
public class ServiceLayer {
    ...
    @Autowired
    public ServiceLayer(GameRepository gameRepository, ConsoleRepository consoleRepository, TShirtRepository tShirtRepository, InvoiceRepository invoiceRepository, TaxRateRepository taxRateRepository, ProcessingFeeRepository processingFeeRepository) {
    ...
    public List<GameViewModel> findGamesByErsbRating(String ersbRating){
        List<Game> games = gameRepository.findByErsbRating(ersbRating);
        List<GameViewModel> gvmList = new ArrayList<>();
        games.stream().forEach(game -> {
            GameViewModel gvm = buildGameViewModel(game);
            gvmList.add(gvm);
    ...
    public TShirtViewModel findTShirt(int id){
        TShirt tShirt = tShirtRepository.findById(id).get();
    ...
    public InvoiceViewModel saveInvoice(InvoiceViewModel invoiceViewModel){
        ...
        if(invoice.getItemType().equals("Consoles")){
            Console item = consoleRepository.findById(invoice.getItemId()).get();
            ...
            if(invoice.getQuantity()>itemQuantity){
                ...
                consoleRepository.save(item);
            }
        }else if(invoice.getItemType().equals("T-Shirts")){
            TShirt item = tShirtRepository.findById(invoice.getItemId()).get();
            int itemQuantity = item.getQuantity();
            itemPrice = item.getPrice();
            // 6. Order quantity must be less than or equal to the number of items on hand in inventory.
            if(invoice.getQuantity()>itemQuantity){
                ...
                item.setQuantity(itemQuantity-invoice.getQuantity());
                tShirtRepository.save(item);
            }
        }else if(invoice.getItemType().equals("Games")){
            Game item = gameRepository.findById(invoice.getItemId()).get();
            int itemQuantity = item.getQuantity();
            itemPrice = item.getPrice();
            // 6. Order quantity must be less than or equal to the number of items on hand in inventory.
            if(invoice.getQuantity()>itemQuantity){
        ...
        BigDecimal processingFee = processingFeeRepository.findByProductType(invoice.getItemType()).getFee();
